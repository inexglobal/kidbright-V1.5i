...
Blockly.JavaScript['mcp7941_rtc_set_datetime_form_ntp'] = function (block) {
	var ssid= block.getFieldValue('ssid');
	var pass = block.getFieldValue('password');
	var timeout = block.getFieldValue('timeout');
	var code = `rtc.syncTime("${ssid}","${pass}",${timeout})`;
	return [code, Blockly.JavaScript.ORDER_NONE];
};
Blockly.JavaScript['mcp7941_rtc_get_DateTime'] = function (block) {
	var code = `rtc.getDateTimeString()`;
	return [code, Blockly.JavaScript.ORDER_NONE];
};

Blockly.JavaScript['mcp7941_rtc_get_Date'] = function (block) {
	var code = `rtc.getDateString()`;
	return [code, Blockly.JavaScript.ORDER_NONE];
};

Blockly.JavaScript['mcp7941_rtc_get_Time'] = function (block) {
	var code = `rtc.getTimeString()`;
	return [code, Blockly.JavaScript.ORDER_NONE];
};
